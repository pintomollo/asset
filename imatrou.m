function [projection, atrous] = imatrou(img, coef, size_max)
...
%
%   SPOTS = IMATROU(IMG, NOISE_THRESH, SPOT_SIZE) performs the "a trous" wavelet
...
%   have no detected spots are set to 1 in order not to prevent the product.
%
%   SPOTS = IMATROU(IMG, NOISE_THRESH) uses the default value MAX(SIZE(IMG)) for
%   SPOT_SIZE.
%
%   SPOTS = IMATROU(IMG) uses the default value of 3 for NOISE_THRES as proposed
%   in [1]. 
%
...
% 13.01.2011
  ...
  for i=1:nplanes
    ...
    noise = (decomposition < coef * 1.4826 * mad(decomposition(:), 1));

...
static void reactions(float *t_fx, float *t_x_prev, float *t_dx, float *t_ddx, float *t_cyto, float *t_params, int t_npts, int t_npops, int t_nparams) {
  ...
  __m128 c00 = _mm_set1_ps(paramsi[1]*(paramsi[5] - t_cyto[0] * paramsi[6]));
  ...
  __m128 c10 = _mm_set1_ps(paramsi[1]*(paramsi[5] - t_cyto[1] * paramsi[6]));
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (nrhs != 9) {
    ...
    for (j = 0; j < niter; ++j) {
      ...
      flag = r4_rkf45(goehring_step, ntotal, x, fx, &t, tmax, &relerr, currerr, prev_flag);

function [b] = myregress(x, y, c, s)
  ...
  if (nargout == 0)
    ...
    %text(x_limits(1) + [0.05; 0.05; ones(ndims,1)*0.05]*range_x, y_limits(end) - [0.1; 0.02; 0.2+(([1:ndims].')/10)]*range_y, [{['R^2 = ' num2str(stats(1))]; ['p_{val} = ' num2str(p_all(:).')]}; cellstr(num2str([b bint]))]);
    text(x_limits(1) + [0.05; 0.05; 0.05]*range_x, y_limits(end) - [0.2; 0.1; 0.02]*range_y, {['R^2 = ' num2str(stats(1))]; ['coeffs = ' num2str(b(:).')]; ['p_{val} = ' num2str(p_all(:).')]});
  end

...
%	  fprintf(fid, '  <DATA');
	  if strcmp(name, 'axlen')
	     fprintf(fid, [uuid '%%  columns="iteration, evaluation, sigma, ' ...
	  ...
	  elseif strcmp(name, 'fit')
	    fprintf(fid, [uuid '%%  columns="iteration, evaluation, sigma, axis ratio, bestever,' ...
		' best, median, worst fitness function value,' ...
		' further objective values of best"' str]);
	  elseif strcmp(name, 'stddev')
	    fprintf(fid, [uuid '%%  columns=["iteration, evaluation, sigma, void, void, ' ...
		'stds==sigma*sqrt(diag(C))"' str]);
	  elseif strcmp(name, 'xmean')
	    fprintf(fid, [uuid '%%  columns="iteration, evaluation, void, ' ...
                          'void, void, xmean"' str]);
	  elseif strcmp(name, 'evol')
	    fprintf(fid, [uuid '%%  columns="iteration, evaluation, fitness, ' ...
...
%             * (repmat(diagC,1,mu) .* arz(:,fitness.idxsel(1:mu)).^2 * weights);
  ...
  if savemodulo && (countiter < 1e2 || ~isempty(stopflag) || ...
	...
	if fid < 1 % err ~= 0 
	  ...
	  elseif strcmp(name, 'fit')
	    fprintf(fid, [uuid '%ld %ld %e %e %25.18e %25.18e %25.18e %25.18e'], ...
	  ...
	  elseif strcmp(name, 'xmean')
	    if isnan(fmean)
	      fprintf(fid, [uuid '%ld %ld 0 0 0 '], countiter, counteval); 
	    else

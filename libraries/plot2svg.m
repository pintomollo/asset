...
if (exist(fullfile(pathstr,'PNG','')) ~= 7)
  mkdir(pathstr, 'PNG');
end
if (exist(fullfile(pathstr,'PNG','')) == 7)
  PLOT2SVG_globals.basefilepath = fullfile(pathstr,'PNG','');
end
%PLOT2SVG_globals.basefilepath = pathstr;
...
if strcmp(get(ax,'Visible'),'on')
    ...
    if axlimy(1)~=axlimy(2)
        if (ticklength(1) ~= 0)
            ...
            xg_label_end = zeros(1,length(axyindex));
            yg_label_end = zeros(1,length(axyindex));
        end
        if ~isempty(axlabely)
...
for i=length(axchild):-1:1
    ...
    elseif strcmp(get(axchild(i),'Type'),'surface')
        ...
        truecolor = (size(pointc, 2) == 3);

        % Scale color if scaled color mapping is turned on, !! If not in true color
        if (~truecolor)
          if strcmp(get(axchild(i),'CDataMapping'),'scaled')
              clim=get(ax,'CLim');
              pointc=(pointc-clim(1))/(clim(2)-clim(1))*(size(cmap,1)-1)+1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function id = layer(fid, close)
global PLOT2SVG_globals

id = PLOT2SVG_globals.runningIdNumber;
...
if (~close)
  fprintf(fid, '<g inkscape:groupmode="layer" id="layer%d" inkscape:label="layer%d" style="display:inline">\n', id, id);
else
  fprintf(fid, '</g>\n');
end

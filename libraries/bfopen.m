function [fname, metadata] = bfopen(fname)
  ...
  for s = 1:numSeries
      ...
      w = r.getSizeX();
      h = r.getSizeY();
      %disp('Warning, extracting only the red channel !');
      numImages = r.getImageCount();
      %numImages = r.getImageCount()/3;

      %disp('!!!!Num Frames Restricted !!!');
      %numImages = 50;
      ...
      movie.data = '';
      for i = 1:numImages
          waitbar(double((s-1)*numSeries + i)/(numSeries*numImages),hwait);
          %if(floor((i/numImages)*10)>imCount)
          %  fprintf('.');
          %  imCount = imCount + 1;
          %end
          try
          ...
          %result = arr;
          %newfile(i).data = uint8(arr);
          newfile = store_data(newfile, double(arr));
      end
      %fprintf('\n');
  end

...
%                   on Varrange
%    fname - Name of the file in which the intermediary results will be saved
%
% Outputs:
...
function [OUT,varargout]=pso_Trelea_vectorized(fitfunc,D,varargin)
...
elseif nargin == 8  % Functname, D, mv, Varrange, minmax, and psoparams, plotfcn, PSOseedValue
   ...
   fname = 'PSO-evol';
elseif nargin == 9  % Functname, D, mv, Varrange, minmax, and psoparams, plotfcn, PSOseedValue, fname
   mv=varargin{1};
   if isnan(mv)
       mv=4;
   end    
...
if (isa(fitfunc, 'function_handle'))
  functname = func2str(fitfunc);
else
  functname = fitfunc;
end
...
uuid = ['PSO' num2str(round(rand(1)*100)) ' '];
...
elseif length(mv)==D     
 velmaskmin = repmat((-(mv(:).')),ps,1); % min vel
 velmaskmax = repmat((mv(:).'),ps,1); % max vel
else
...
posmaskmeth = 3; % 3=bounce method (see comments below inside epoch loop) %% 2 = wrap around

dp = (posmaskmax - posmaskmin);
dv = (velmaskmax - velmaskmin);
...
% VECTORIZE THIS, or at least vectorize cost funct call 
 out = feval(fitfunc,pos);  % returns column of cost values (1 for each particle)
%---------------------------
    ...
    if plotflg==1      
      if (rem(i,df) == 0 ) | (i==me) | (i==1) 
        ...
        %end
         ...
         fprintf(fid, [uuid '%ld : %ld |'], i, pbestval(idx1));
         fprintf(fid, ' %f',pbest(idx1,:));
         fprintf(fid, '\n');
         fclose(fid);
       ...
       %         2) wraparound at limit , 3) bounce off limit
        ...
        elseif posmaskmeth == 2
         % this is the wraparound method
          ...
          pos = ( minposmask_throwaway.*(posmaskmax - mm) ) + ( minposmask_keep.*pos );
          pos = ( maxposmask_throwaway.*(posmaskmin + mm) ) + ( maxposmask_keep.*pos );                
        elseif posmaskmeth == 3
         % this is the bounce method, particles bounce off the boundaries
         % with -vel
          mm = mod(pos, dp);

          pos = ( minposmask_throwaway.*(posmaskmin + mm) ) + ( minposmask_keep.*pos );
          pos = ( maxposmask_throwaway.*(posmaskmax - mm) ) + ( maxposmask_keep.*pos );

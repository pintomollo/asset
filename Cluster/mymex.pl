...
sub compile_files
{
    ...
    if ($main::libNotFound) {
        print "\nWarning: MEX could not find the library \"$main::libNotFound\" " .
    ...
    for (;$file = shift(@FILES);) {
        ...
        if ($FILEEXT =~ /($COMPILE_EXTENSION)$/i ) {
            ...
            my ($args) = "$ARG_FLAGS $COMPFLAGS $name_arg $FLAGS " . smart_quote(mexGetShortPathPreservingFilename(File::Spec->rel2abs($file)));
...
sub linker_arguments
{
    ...
    if ( $ENV{'RSP_FILE_INDICATOR'} )
    {
        ...
        $ARGS = "$NAME_OUTPUT $LINKFLAGS " .
                ...
                mexGetShortPathName($response_file) .
...
sub options_file
{
    ...
    } elsif ($OPTFILE_NAME =~ /msvc90free/ && $ENV{'MWE_VS90FREE_COMNTOOLS'}) {
        print("Setting VS90COMNTOOLS for use with Microsoft Visual Studio 2008 Express Edition (MathWorks-only diagnostic - do not geck)\n");
        $ENV{'VS90COMNTOOLS'} = $ENV{'MWE_VS90FREE_COMNTOOLS'};
    }
...
sub parse_common_dash_args
{
    ...
    ARGTYPE: { 
      ...
      /^-L(.*)$/ && do {
          push(@IMPLICIT_LIB_DIRS, mexGetShortPathName($1));
          last ARGTYPE;
      };
      ...
      /^-f$/ && do {
        ...
        if (-e $filename) {
            $OPTFILE_NAME = mexGetShortPathName($filename);
        } else {
      ...
      /^-output$/ && do {
          ...
          if ($mex_ext eq ".dll" && $ARCH eq "win32") {
              $mexext_was_dll = true;
          }
      ...
      /^-outdir$/ && do {
        $outdir_flag = "yes";
        # OUTDIRN is passed to MIDL.EXE by mbuild.
        my $outdirn = mexCanonpath(shift(@ARGV));
        $ENV{'OUTDIRN'} = $outdirn;
        ...
        $ENV{'OUTDIR'} = mexGetShortPathName($outdir) . "\\";
        last ARGTYPE;
      };
...
sub set_common_variables
{
    ...
    if (!$ENV{'OUTDIR'})
    {
        ...
        {
            $ENV{'OUTDIR_LONGPATH'} = File::Spec->rel2abs($link_outdir) . "\\";
            $ENV{'OUTDIR'} = mexGetShortPathName(File::Spec->rel2abs($link_outdir)) . "\\";
        }
    }
...
end_wrong_mexext_in_output_flag
    ...
    /^dll_is_going_away_warning$/ && print(<<"end_dll_is_going_away_warning") && last DESCRIPTION;

  Warning: The "dll" extension for 32-bit Windows MEX-files is being phased
           out. This MEX-file was specified with a "dll" extension and will 
           not work in future versions of MATLAB. Use "mexw32" or no extension 
           at all.
           
end_dll_is_going_away_warning
...
end_bad_link_option_mb
   ...
   /^bad_env_option$/ && print(<<"end_bad_env_option_mb") && last DESCRIPTION;

  Incorrectly formatted argument to -env switch. The -env switch must be
  followed by a variable assignment statement, such as MCR_MAJOR_REV=8.

end_bad_env_option_mb
     do {
         print "Internal error: Description for $_[0] not implemented\n";
         last DESCRIPTION;
     };
...
sub parse_mbuild_args
{
    ...
    for (;$_=shift(@ARGV);) {
        ...
        ARGTYPE: {
          ...
          /^-env$/ && do {
              $envAssign = shift(@ARGV);
              if ($envAssign =~ /^[A-Za-z0-9_]+[#=].*$/)
              {
                  push(@SET_ENV_VARIABLES, $envAssign);
              }
              else
              {
                  describe_mb("bad_env_option");
              }
              last ARGTYPE;
          };
...
sub register_dll
{
    ...
    # register_dll: Register DLL with COM object system.
    #===================================================================
    # Use the longpath to register the object.
    my $dllpath = mexCatfile($ENV{'OUTDIR_LONGPATH'}, "$ENV{'MEX_NAME'}.$bin_extension");
...
sub post_mex_warnings
{
    ...
    if ($mexext_was_dll) {
        describe("dll_is_going_away_warning")
    }
...
else
{
    ...
    process_env_assignments(\@SET_ENV_VARIABLES);

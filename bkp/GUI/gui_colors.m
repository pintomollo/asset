...
% 28.08.14
  ...
  if (~ischar(mycolormap))
    mycolormap = func2str(mycolormap);
  end
  ncolors = 128;
  ...
  for i=1:length(colors.colormaps)
    if (ischar(colors.colormaps{i}))
      if (strcmp(mycolormap, colors.colormaps{i}))
        mycolormap = i;
        break;
      end
    else
      if (strcmp(mycolormap, func2str(colors.colormaps{i})))
        mycolormap = i;
        break;
      end
    end
  ...
  function hFig = create_figure(list)
    ...
    for i=length(list):-1:1
      ...
      if (ischar(list{i}))
        values = brewermap(ncolors, list{i});
        fstr = list{i};
      else
        values = list{i}(ncolors);
        fstr = func2str(list{i});
      end

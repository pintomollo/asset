...
function hFig = create_figure(data)

  % Fancy naming
  if (isempty(data.name))
    ftitle = 'Edit options';
  else
    ftitle = ['Edit options (' data.name ')'];
  end
...
function recursive_edit(hObject, eventdata, handles)
  ...
  if (isempty(data.name))
    ...
    fname = [data.name '.' fieldname];
  end

  % Edit the substructure
  [value, is_updated] = edit_options(data.mystruct.(fieldname), fname);

  % Store the new values
  data.mystruct.(fieldname) = value;
  data.is_update = is_updated;
  guidata(data.hFig, data);

  % Display the old panel
  set(data.hFig, 'Visible', 'on');

...
% 14.05.2014
  ...
  if (nargin == 0 || isempty(fname) || ...
    ...
    [fname, metadata, opts] = convert_movie();
  end
...
function update_display(data, recompute)
...
% of its content.
  ...
  colormap(handles.axes(1), colors.colormaps{color_index});
  colormap(handles.axes(2), colors.colormaps{color_index});

  % Save the data back
  data.img = img;
  data.orig_img = orig_img;
  data.img_next = img_next;
...
% This function adds a new channel to the current recording
  ...
  if (~isempty(new_channel))
    ...
    if (nchannels == 0 || nframes == curr_nframes)
      ...
      % Channel name
      if (numel(new_channel) > 0)
        [junk, cname, junk] = fileparts(new_channel);
        [junk, cname, junk] = fileparts(cname);
        indx = find(cname=='_', 1, 'last');
        if (numel(indx)>0)
          cname = cname(indx+1:end);
        end
        cname = [num2str(length(channels)) ' - ' cname];
      else
        cname = ['Channel ' num2str(length(channels))];
      end
...
% This function stops the current processing after confirmation
  ...
  if (strcmp(get(data.hFig, 'Visible'), 'off'))
    ok = true;
  else
...
% and linking the callbacks.
  ...
  for i = 1:nchannels
    ...
    if (numel(channels(i).fname) > 0)
      [junk, cname, junk] = fileparts(channels(i).fname);
      [junk, cname, junk] = fileparts(cname);
      indx = find(cname=='_', 1, 'last');
      if (numel(indx)>0)
        cname = cname(indx+1:end);
      end
      cname = [num2str(i) ' - ' cname];
    else
      cname = ['Channel ' num2str(i)];
    end
    liststring = [liststring cname];
    if (i < nchannels)
  ...
  indx = find(exp_name=='_', 1, 'last');
  if (numel(indx)>0)
    exp_name = exp_name(1:indx-1);
  end
